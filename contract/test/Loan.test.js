...
describe("zkRisk Loan Contract", function () {
    ...
    async function deployLoanFixture() {
        ...
        // Deploy mock tokens with 6 decimals for USDC and 18 for SHIB
        const MockERC20 = await ethers.getContractFactory("MockERC20");
        // Pass arguments directly to deploy(), not in overrides
        const usdc = await MockERC20.deploy("USD Coin", "USDC", 6); // 6 decimals like real USDC
        const shib = await MockERC20.deploy("Shiba Inu", "SHIB", 18); // 18 decimals like real SHIB

        // Deploy mock Pyth oracle
        const MockPyth = await ethers.getContractFactory("MockPyth");
        const mockPyth = await MockPyth.deploy();

        // Deploy mock SelfProtocolBridge (without Hyperlane dependencies)
        const MockSelfProtocolBridge = await ethers.getContractFactory("MockSelfProtocolBridge");
        const mockBridge = await MockSelfProtocolBridge.deploy(owner.address);

        // Deploy X402Payment with mock USDC
        const X402Payment = await ethers.getContractFactory("X402Payment");
        const x402Payment = await X402Payment.deploy(await usdc.getAddress());

        // Deploy PythVolReader with mock Pyth
        ...
        // Deploy main Loan contract with mock dependencies
        ...
        // Mint test tokens
        await usdc.mint(user1.address, ethers.parseUnits("10000", 6)); // 10k USDC with 6 decimals
        await shib.mint(user1.address, ethers.parseUnits("1000000000", 18)); // 1B SHIB with 18 decimals
        await usdc.mint(await loan.getAddress(), ethers.parseUnits("100000", 6)); // Liquidity with 6 decimals
